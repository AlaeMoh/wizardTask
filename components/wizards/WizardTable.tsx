...
const { data: wizards, isLoading, isError } =
  useWizards(debouncedSearch);

const filtered = wizards?.filter((wizard) => {
  const name = getWizardName(wizard)?.toLowerCase() || "";
...
const paginated = filtered.slice(
  ...
  return (
    <>
      <WizardDetailModal wizard={selectedWizard} onClose={() => setSelectedWizard(null)} />
        ...
        >
          ...
          <span className="table-title">Master Wizard Registry</span>
            <p style={{ fontSize: "12px", color: "var(--text-muted)", marginTop: "2px" }}>
              {wizards ? `${wizards.length} wizard${wizards.length !== 1 ? "s" : ""} found` : "Loading..."}
            </p>
          </div>
        ...
        <div className="table-wrapper">
          <table className="wizard-table">
            <thead>
              ...
              >
                {["ID", "Name", "First Name", "Last Name", "Elixirs", "Actions"].map((col) => (
                  <th
                    key={col}
                    style={{
                      padding: "10px 16px",
            ...
            <tbody>
                ...
                <tr>
                  <td colSpan={4} style={{ padding: "48px 16px", textAlign: "center" }}>
                    ...
                    <p style={{ color: "var(--text-muted)", fontSize: "12px", marginTop: "4px" }}>
                      {debouncedSearch ? `No results for "${debouncedSearch}"` : "The registry appears to be empty"}
                    </p>
                  </td>
                </tr>
              ) : (
             ...
             <td> {`${wizard.id.slice(0, 8)}-...-${wizard.id.slice(-6)}`}</td>
                    ...
                    <td style={{ padding: "12px 16px", fontSize: "13px", color: wizard.firstName ? "var(--text-secondary)" : "var(--text-muted)" }}>
                      {wizard.firstName ?? "—"}
                    </td>
                    <td style={{ padding: "12px 16px", fontSize: "13px", color: wizard.lastName ? "var(--text-secondary)" : "var(--text-muted)" }}>
                      {wizard.lastName ?? "—"}
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                        <span
                          style={{
                            ...
                            padding: "0 6px",
                            borderRadius: "6px",
                            background: wizard.elixirs.length > 0 ? "rgba(108, 140, 255, 0.12)" : "var(--bg-elevated)",
                            border: `1px solid ${wizard.elixirs.length > 0 ? "rgba(108, 140, 255, 0.25)" : "var(--border-subtle)"}`,
                            fontSize: "11px",
                            fontWeight: 600,
                            color: wizard.elixirs.length > 0 ? "var(--accent-primary)" : "var(--text-muted)",
                          }}
                        >
                   ...
                   <td>
                  <button className="action-btn" onClick={() => setSelectedWizard(wizard)}>
        ...
        {!isLoading && !isError && (wizards?.length ?? 0) > 0 && (
          ...
          >
            <span style={{ fontSize: "12px", color: "var(--text-muted)" }}>
              Showing {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, wizards?.length ?? 0)} of {wizards?.length ?? 0}
            </span>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      height: "30px",
                      borderRadius: "6px",
                      border: page === p ? "1px solid var(--accent-primary)" : "1px solid var(--border-default)",
                      background: page === p ? "rgba(108, 140, 255, 0.15)" : "var(--bg-elevated)",
                      color: page === p ? "var(--accent-primary)" : "var(--text-secondary)",
                      cursor: "pointer",
                      fontSize: "12px",
